...
use serde_json::json;
use thingr::{ClaimRequest, FilesTaskStore, NewTask, Task, TaskStatus, TaskStore, TransitionTask};
...
mod tests {
    ...
    use brunnr_core::{AgentCatalogEntry, AgentModel};
    ...
    fn parses_brunnr_and_claude_role_definitions() {
        let tempdir = TempDir::new("hird-definitions");
    ...
    async fn unavailable_agent_or_model_fails_before_spawn() {
        let tempdir = TempDir::new("hird-unavailable");
    ...
    async fn plan_approval_gate_blocks_until_review_approves() {
        let tempdir = TempDir::new("hird-plan-gate");
        let mut runtime = TeamRuntime::new(runtime_config(
            tempdir.path(),
            vec![
    ...
    async fn team_message_redacts_successful_secret_output() {
        let tempdir = TempDir::new("hird-secret-output");
    ...
    async fn team_run_cleans_up_process_group_children() {
        let tempdir = TempDir::new("hird-cleanup");
